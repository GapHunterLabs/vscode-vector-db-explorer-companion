 ...
 * a real number of successful Qdrant collection list refreshes -- never on install, never on a timer.
...
const KEY_HIT_COUNT = 'vectorDbExplorerCompanion.review.hitCount';
const KEY_ANSWERED = 'vectorDbExplorerCompanion.review.answered';
...
/**
 * Call this once per real successful Qdrant collection list refreshes. Safe to call multiple times; a
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Vector Database Explorer Companion: you've used this ${HITS_BEFORE_PROMPT} times -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
